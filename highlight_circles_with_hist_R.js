...
data_to_bind2 = [{x:100, y:100, trend: [{x:'30/7/13',y:100}, {x:'30/7/14',y:300}, {x:'30/7/15',y:300}], val:"abba", grp:"a"}, 
				{x:200, y:150, trend: [{x:'30/7/13',y:100}, {x:'30/8/14',y:290}, {x:'30/7/15',y:300}], val:"bc", grp:"a"}, 
				{ x:300, y:170, trend: [{x:'30/7/13',y:100}, {x:'30/7/14',y:201},{x:'30/7/15',y:300}], val:"aoo", grp:"b"}, 
				{x:400, y:131, trend: [{x:'30/7/13',y:100}, {x:'30/9/14',y:133}, {x:'30/7/15',y:300}], val:"oxox", grp:"c"}]
...
right_vert_x_upper = true_width + axis_width  // right hand upper side
right_vert_y_upper = 10
right_horiz_x_upper = true_width + axis_width
right_horiz_y_upper = (height / 2) - axis_height

right_vert_x_lower = true_width + axis_width  // right hand lower side
right_vert_y_lower = (height / 2) + 10
right_horiz_x_lower = true_width + axis_width
right_horiz_y_lower = height - axis_height
...
var scaleXscatter = d3.scaleLinear()
	.domain([0, 300])
	.range([0, plot_width/2])

var scaleYscatter = d3.scaleLinear()
	.domain([300, 0])
	.range([0, (plot_height/2) - axis_height + right_vert_y_upper*2])
...
svg.append("g")            /// making axes
	...
	.attr("transform", "translate("+right_horiz_x_upper+","+right_horiz_y_upper+")")
	.selectAll("text")
		.attr("transform", "translate(0, 20) rotate(-60)") 

svg.append("g")
	.call(d3.axisLeft(scaleYscatter))
	.attr("transform", "translate("+right_vert_x_upper+","+right_vert_y_upper+")")
...
// getting unqiue vals
var unique_vals = [...new Set(all_dates)]   /// gets unique grp values

scaleXhist = d3.scaleOrdinal()
  .domain(unique_vals)
  .range(plot_width)

var barWidth = Math.ceil(plot_width / unique_vals.length);



//// START FUNC HERE: works outside but not inside function atm
//// might be to do with svg.appendAll() not liking working inside a function
//function draw_hist(hist_vals) {  

//// try histogram code: https://www.d3-graph-gallery.com/graph/histogram_basic.html

  
  
hist_vals =  [{x:'30/7/13',y:100}, {x:'30/7/14',y:300}, {x:'30/7/15',y:300}]
//

hist_vals = d3.map(hist_vals, function(d){return d.x})
console.log(hist_vals)

var bars = []
for (i = 0; i < unique_vals.length; i++) {
  if (hist_vals.includes(unique_vals[i])) {
    bars.push(1)
  } else {
    bars.push(0)
  }
...
svg.selectAll('rect')
  ...
  .append('rect')
    .attr('width', barWidth)
    .attr('x', function(d, i) { return right_horiz_x_lower + (i * barWidth); })
    .attr('height',  function(d) { return (d * plot_height)  ; })
    .attr('y', right_vert_y_lower )
    .attr('fill', 'steelblue');
//}
